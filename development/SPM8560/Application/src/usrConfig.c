...
//extern UINT32 sysClkFreqGet();
#define __GDEF_H__				/* dont let bcm sal header file redefine uint32 and uint64 */

#undef INCLUDE_TFFS				/* Dont let usrDepend.c include the sysTffs.c file */
...
#define CCI_TASK_PRIORITY 20
#define USE_SOFTWARE_DEFAULT

#include <usrNetCciInit.c>
...
volatile PDA    *gIoMem;    /* Ptr to MPC8560 memory map structure  */
...
#ifdef PROJECT_SPECIFIC_USRNETWORK
  #include "usrNetwork.c"
#endif

#include "private/symLibP.h"  /* for SYMTAB */

#if !defined(TARGETDEV_SIMPC) && !defined(TARGETDEV_SIMSO)
#include "utilities.h"
#endif
...
/* bcm5645 (broadcom 5645) */
#if defined(INCLUDE_BCM1)
extern void bcm1_init(void);
extern void SwitchTestInit(void);
#endif

#ifdef INCLUDE_BCM5324
extern void bcm5324SwitchInit(void);
extern void bcm5324PortBasedVlanCreate(UINT32 portMap);
#endif

#ifdef INCLUDE_SECURITY_LIBRARIES_IPSEC_IKE_INIT
extern void secLibInit(void);
extern STATUS ipsecComponentsInit(void);
#endif

#ifdef INCLUDE_WEB1_INIT
extern void web1Dummy(void);
#endif

#if defined(INCLUDE_NWT)
extern void nwtInit(void);
#endif

#if defined(INCLUDE_NMT)
extern void nmtInit(void);
#endif

#ifdef TELLABS_FLASHDRVR
//extern int32 flash_init(void (*console_print)(char *),void (*delay_func)(unsigned int), unsigned int delay_arg );
#endif
...
/* retrieve number of 256MB blocks of memory available. 
   This routine is only available on the SPM */
extern int getBrdPhyMemSize();
...
extern startVxWorksPbootShell();
#endif


extern UINT32 rtcDecTimeConst;
extern void DecIsr(void);
extern int milliseconds2Tick;
extern UINT32 sysTimestampFreq (void);
extern UINT32 nextTBL;


extern void tlabSpecificRebootHook (int);
extern void handleFatalKernelError (int, EDR_TASK_INFO *, bool);
extern EDR_ERRINJ_HOOK_FUNCPTR handleFatalUserError(int,const char *, int,void *, const char *);

extern int sysStaticTlbDescNumEnt_4GB;
extern int sysStaticTlbDescNumEnt;
extern int sysPhysMemDescNumEnt;
extern int sysPhysMemDescNumEnt_4GB;
static volatile const char *dummy_cp;


#pragma use_section DATA SwitchDmaBufferLocation, SwitchDmaBufferSize, driverLogOldLocation, driverLogOldSize, driverLogLocation, driverLogSize; 
...
void usrInit
    ...
    {
    int numMemBlk;
    int bstopre, ddr_sdram_interval;
	dummy_cp = patchesNotNeeded[0]; // DO NOT DELETE! This pulls the patchesNotNeeded[] table into the load.
    ...
    //---------------------------------------------------------
    // The SPM8560 has always shipped with DDR RAM that supports page-mode access,
    // but the SPM8560's BBOOT configured the DDR RAM to operate in global auto
    // precharge mode (DDR_SDRAM_INTERVAL[BSTOPRE] = 0), also known as closed-page
    // mode.  In closed-page mode, the DDR controller must issue a precharge command
    // for EACH AND EVERY ACCESS.  In page mode, the DDR controller leaves RAM pages
    // open for some period of time (defined by BSTOPRE) under the assumption that
    // a subsequent access may hit that already-open page, saving a few clock cycles
    // on each subsequent access.  Tests showed a 100% to 170% speed improvement
    // when BSTOPRE is set to any nonzero value (enabling page-mode access).
    //
    // Ideally this change would be implemented in BBOOT, where all RAM configuration
    // takes place.  However, doing so would require us to upgrade BBOOTs in the
    // field, which is dangerous.  Also, if we did that, and an upgraded SPM was
    // later inserted into a system with an older feature package, that would affect
    // the performance of the older feature package in untested ways.
    //
    // The solution is to change BSTOPRE=32 in PBOOT and APPL, where it will affect
    // only the current feature package.  Note that it must be set in *both* PBOOT
    // and APPL, since in some cases, BBOOT jumps straight to APPL without going
    // through PBOOT.
    //
    ddr_sdram_interval = (*((volatile UINT32 *) (DDRBA + DDR_SDRAM_INTERVAL))) & ~0x3FFF;
    bstopre = 32;
    *((volatile UINT32 *) (DDRBA + DDR_SDRAM_INTERVAL)) = ddr_sdram_interval | bstopre;

    BootKickWatchdogs();

    //adjust number of entries according to number of 256MB block of memory
    /*
       1 = 256MB	9 = 2.25GB
       2 = 512MB	A = 2.50GB
       3 = 768MB	B = 2.75GB
       ...
       8 = 2GB       	
    */
    numMemBlk = MAX_LOCAL_MEM_256_BLK - getBrdPhyMemSize();
    sysPhysMemDescNumEnt -= numMemBlk;
    sysStaticTlbDescNumEnt -= numMemBlk;
...
#if !defined(TARGETDEV_SIMPC) && !defined(TARGETDEV_SIMSO)    
//TTRAN - TEst only: This will place the MAC Addr into RAM. Simulate EEPROM content.
// moved into usrRoot - too early for this call	setEnetAddr_8560();  
    setUpPIO();
#endif
...
void usrRoot
    ...
    UINT32 bcm_separation_timestamp = TMBget_timestamp();
    int rc;
 ...
 */
    ...
    if (! CheckRealDevelopmentJumper() )
    {
         /* Since we have alot of memory to configure in the MMU, we will
            extend the watchdog timer for around 60seconds. This should be plenty
            of time for clearing memory for 3.5GB */
            setUpWatchdog1stTime(60000); 
    }
...
# endif	/* defined(_WRS_OSM_INIT) */
    ...
    rtcDecTimeConst = sysTimestampFreq()/(sysClkRateGet()*DEC_TICKS_PER_VX_TICKS);    
    milliseconds2Tick = 1000/( sysClkRateGet())/DEC_TICKS_PER_VX_TICKS;
    nextTBL = sysTimeBaseLGet() + rtcDecTimeConst;
    ...
    if (! CheckRealDevelopmentJumper() )
    {
        /* After MMU config, we will set the CPU watchdog to the real timeout value 10 sec*/
        setUpWatchdog2ndTime(10000); 
    }
    else
    {
        /* 
         * Create the watchdog high priority task even if the DEV jumper is present. 
         * This will allow the task to print debug information if the software watchdog
         * is not being punched.  NOTE: when the DEV jumper is present, the watchdog
         * high priority task will not reset the card. It will only display/store
         * debug information.
         */
        watchdogStartKicker();
    }
#ifdef PBOOT
        createPbootSWTimerTask();
#endif
 ...
 */
  cacheCreateInternalDataRAM((UINT32 *)FD_ORIGIN, FD_NUMLINES);
    ...
    /*---------------------------------------------*/
    gIoMem = (PDA *)BRD_IO_MAP_ADDR;
    /*----------------------------------------------------------*/
    /* Calculate the rate at which the decrementer will run.    */
    /* This is normally 1/4 the CPU internal clock rate.        */
    /* i.e is 333Mhz / 4.                                        */
    /*----------------------------------------------------------*/    
    BoardDecClkRateHz = (BD_INPUT_CLOCK / 4);
    ...
    retVal = SPI_init(0);
#endif
    // Register the Storage and ID EEPROM Devices     
    eepromIdRegister();
    eepromStorageRegister();    
    eepromBpRegister();
    eepromAipRegister();    
    eepromIdRemoteRegister_25040();
    eepromIdRemoteRegister_25160();
    getPartNumber(); // This sets the gCardtype_SPM and gCardtype_DPM bools.
...
n", dectime[3], dectime[5], dectime[6], dectime[2], dectime[1], dectime[0]);
#endif
#endif // INCLUDE_I2C

    /* Configure DS75 Temperature sensors */
    //!+! should be initialized by application
    Ds75Config();
    ...
    /* Tellabs change - Disable flow control to prevent errant ctrl-s from halting */
    /*                  the SPM during bootup.                                     */
	(void) ioctl (consoleFd, FIOSETOPTIONS, (OPT_TERMINAL & (~OPT_TANDEM)));
	}
	...
	for (ix = 0; ix < NUM_TTY; ix++)	/* create serial devices */
#else
/* since only SCC 4 is enabled, we will hard code the number 3 below. */
	for (ix = 3; ix < NUM_TTY; ix++)	/* create serial devices */
#endif
	    {
		...
		continue;
#endif
		memset(tyName,0x00,20);
//	    sprintf (tyName, "%s%d", "/tyCo/", ix);
 /* we are going to hard code the tty name to /tyCo/0 for port 3*/
		memcpy(tyName,"/tyCo/0",7);
...
# endif	/* INCLUDE_LOG_STARTUP */
	...
	STATUS cfStat = ERROR;
	    ...
	    if ((cfDrv(0,   // ctrl
		       1,   // number of drives on this controller
		       CF_PIO_MULTI, // configType
		       CS1_BASE + 0x1000,
		       CS1_BASE + 0x1000))
		== ERROR)
	    {
		printf ("cfDrv() returned ERROR from usrRoot!\n");
	    }
	    else
	    {
		printf("Compact Flash:\n");
		cfPrintDriveInfo(0,0);
	    }

	    cfStat = diskMountAllPartitions(0,0);
	...
	if (cfStat != OK)
        {
	    printf("\n#######################################################"
		   "\n# ERROR detecting or initializing Compact Flash disk: #"
		   "\n#    COMPACT FLASH FILESYSTEMS ARE UNAVAILABLE!!!     #"
	           "\n#######################################################\n\n");

            diskLog_print(); // Print the disk log in case it might help determine the problem.
        }
#endif /* INCLUDE_STDIO */
	}
	diskStatsLog_init(); // Re-init the stats & clear the disk log
	initSectorzeroDebug(); // SECTORZERO_DEBUG stuff; see diskUtils.c (pseudo-temporary)
...
#ifdef INCLUDE_BCM5324
    bcm5324SwitchInit();  
    bcm5324PortBasedVlanCreate(0x01010000);  /* zero based port 16 and and zero based port 24 */
#endif

	TlabParamInit();	/* Initialize BspVars, AppVars, and SysVars */
    InitCpldWatchdog(); /* Disable the CPLD watchdog if virtual development jumper is enabled. */
/*
...
# endif	/* INCLUDE_LOGGING && INCLUDE_LOG_STARTUP */
    ...
    if (standTblSize > 0)
    {
        ...
        //memShow(0); /* Use this to print out memory usage right before loading symbols */
        start_time = time(NULL);
        /* load system symbol table */
        usrStandaloneInit ();
        finish_time = time(NULL);
        printf ("%u symbols loaded in %u seconds\n", sysSymTbl->nsymbols, finish_time - start_time);
        //memShow(0); /* Use this to print out memory usage right after loading symbols */
    }
#endif  /* INCLUDE_STANDALONE_SYM_TBL */

#if (defined(INCLUDE_NET_SYM_TBL) || defined(INCLUDE_PASSFS_SYM_TBL))
    if (sysSymTbl->nsymbols == 0)
        usrLoadSyms();
...
#ifdef APPL
/* Tellabs Addition: add support for crash log mechanism */
crashInitCommon();
#endif
...
//    printf (" CPU: %s.  Processor #%d.\n", sysModel (), sysProcNumGet ());
	...
	edrSystemDebugModeSet (TRUE);	/* set to lab mode */
	/*Add EDR hook to reset card on fatal exception*/
	edrPolicyHandlerHookAdd((VOIDFUNCPTR)handleFatalKernelError); 
	/* Add EDR hook to reset card on user exception (generated due to cluster problem) */
	edrErrorInjectHookAdd((EDR_ERRINJ_HOOK_FUNCPTR)handleFatalUserError); 
...
//TMBmsec_delay(taskDelayCount);

/* bcm5645 (broadcom 5645) */
#if defined(INCLUDE_BCM1)
    while (TMBmsec_since(bcm_separation_timestamp) < 2000)
    {
     /* Enforce a 2000 millisecond separation between bcm1_init() and
      * bcm5324SwitchInit() far above.  In testing, we found that if we call
      * bcm1_init() less than 1270 milliseconds after completing bcm5324SwitchInit(),
      * we would sometimes (quite often or always on some modules) experience heavy
      * link degradation (5 min to FTP a file that should only take 30 seconds).
      * For safety, we enforce a 2000 millisec separation, delaying if necessary.
      */ ;
    }
    bcm1_init();
#endif
...
#ifdef TELLABS_FLASHDRVR
	if (! CheckRealDevelopmentJumper() ) //unprotect flash
	    *(unsigned char *)(BRD_CPLD_REG_BASE + 6) = *(unsigned char *)(BRD_CPLD_REG_BASE + 6) & 0x7f;
 	flash_init(printf,taskDelay,10);
#endif

//#ifdef INCLUDE_TFFS   
#ifdef ONBOARD_STORAGE
     setupFileSystem();
#endif
...
/*  usrMmuOptimize needs to be called before the additional CPU's are enabled.*/
    ...
    /*Tellabs Change*/
    /* Add custom reboot hook*/
#ifdef APPL
    /*Add reboot hook */
    rebootHookAdd((FUNCPTR)tlabSpecificRebootHook);
#endif
...
#define RTP_APPL_CMD_SCRIPT_FILE NULL
    ...
    /*We have two choices here: Start the legacy pboot or the vxworks pboot shell*/

#ifdef PBOOT
    if (startVxWorksPbootShell() == 1)
    {
        usrShell ();
    }
    else
#endif
    {
	reconfigWdt(20000);  /*Reset the WDT to 20secs for loading the application code. */
	                     /* Once app comes up, it will set it back down to something a lot smaller */
...
void usrClock ()
    ...
    {
       DecIsr();    /* announce system tick to kernel */
    }
...
/* Temporary stubs for pesky routines we can't resolve mjk */
