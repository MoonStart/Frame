...
Copyright(c) Tellabs Transport Group. All rights reserved
 ...
 AUTHOR   :     19 May 2004  Hemant Thakkar
 DESCRIPTION:   Update and keep the protection switching 
                statistics of OCH layer
--------------------------------------------------------------------------*/

#ifndef PM_BBSTATUSOTU_H
#define PM_BBSTATUSOTU_H
...
//Keeps various OCH switching statistics up to 
//date for the current second. Note that unlike the PM_BbStatus classes
//of other layers such as OPT, the Otu does not derive from the base
//class PM_BbStatus. The purpose of PM_BbStatus and the status classes, in
//general, has been to hold status of guage type parameters. In Otu
//case, however, the status information is primarily for singal protection
//status information. 
class PM_BbStatusOtu : public FC_BbObject
{
public:
...
//-----------------------------------------------------------------
inline uint32 PM_BbStatusOtu::GetPSC() const
{
    return myPSCCounter.GetCount();
}

//-----------------------------------------------------------------
inline uint32 PM_BbStatusOtu::GetPSD() const
{
    return myPSDCounter.GetCount();
}

//-----------------------------------------------------------------
inline bool PM_BbStatusOtu::EverPresentRevertiveMode()
{
    return myCurrentRevertiveMode;
}

//-----------------------------------------------------------------
inline bool PM_BbStatusOtu::EverRemovedFromProtectionScheme()
{
...
//-----------------------------------------------------------------
inline bool PM_BbStatusOtu::EverPresentInProtectionScheme()
{
    return myCurrentProtectionShemePrsenceStatus;
}

//-----------------------------------------------------------------
inline bool PM_BbStatusOtu::EverChangedProtectionScheme()
{
...
//-----------------------------------------------------------------
inline bool PM_BbStatusOtu::EverChangedRevertiveMode()
{
